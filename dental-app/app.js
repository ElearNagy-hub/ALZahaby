...
const I18N = {
  en: {
    langBtn: "العربية",
    langCode: "AR",
    docTitle: "Al-Dahabi Dental Supplies | Expert Clinical Partner",
    logoTitle: "Al-Dahabi Dental",
    ...
    heroCtaWhatsApp: "Order via WhatsApp",
    ...
    trust2: "Founders: Licensed Dentist + Pharmacist",
    ...
    footerBrandMain: "Al-Dahabi Dental Supplies",
    footerBrandSub: "Kasr Al-Ainy Market, Cairo",
    footerTagline: "Expert-Led Professional Service & Value for Money — Bridging consumables & equipment since Phase 1.",
    footerShop: "Shop & Categories",
    footerEquipment: "Dental Devices & Equipment",
    footerMaterials: "Materials & Consumables",
    footerHomeCare: "Home Care Products",
    footerStudentKits: "Student Ambassador Kits",
    footerServices: "Services",
    footerFreeConsult: "Free Clinical Consultation",
    footerWholesale: "Wholesale Quotes for Clinics",
    footerMaintenance: "Maintenance & Installation",
    footerAmbassadors: "University Ambassador Program",
    ...
    footerRights: "© 2026 Al-Dahabi Dental Supplies. All Rights Reserved. | MVP Phase 1",
    legalSingleLLC: "Single-Member LLC",
    legalEda: "EDA Registered",
    legalCommercial: "Commercial Register & Tax Card",
  ...
  ar: {
    langBtn: "English",
    langCode: "EN",
    docTitle: "شركة الذهبي لمستلزمات وأجهزة الأسنان | التميز الإكلينيكي",
    logoTitle: "شركة الذهبي",
    ...
    footerBrandMain: "شركة الذهبي لمستلزمات وأجهزة الأسنان",
    footerBrandSub: "سوق قصر العيني لمستلزمات الأسنان، القاهرة",
    ...
    footerEquipment: "أجهزة ومعدات الأسنان",
    footerMaterials: "المواد والمستلزمات",
    footerHomeCare: "منتجات العناية المنزلية",
    footerStudentKits: "باقات سفراء الطلاب",
    footerServices: "الخدمات",
    footerFreeConsult: "استشارة إكلينيكية مجانية",
    footerWholesale: "عروض أسعار الجملة للعيادات",
    footerMaintenance: "خدمات الصيانة والتركيب",
    footerAmbassadors: "برنامج سفراء الجامعات",
...
function getStatSuffix(el, lang) {
  if (lang === 'en') {
    return el.dataset.suffixEn !== undefined ? el.dataset.suffixEn : (el.dataset.suffix || '');
  }
  return el.dataset.suffixAr !== undefined ? el.dataset.suffixAr : (el.dataset.suffix || '');
}
...
function setLanguage(lang) {
  ...
  // Update stat numbers suffix according to language
  qsa('.stat-number[data-count]').forEach(el => {
    const target = parseInt(el.dataset.count);
    const suffix = getStatSuffix(el, lang);
  ...
  const floatWaBtn = qs('#whatsapp-float-btn');
  if (floatWaBtn) {
    const msg = isAr
      ? encodeURIComponent("مرحباً شركة الذهبي لمستلزمات الأسنان، أود الاستفسار.")
      : encodeURIComponent("Hello Al-Dahabi Dental Supplies, I have an inquiry.");
    floatWaBtn.href = `https://wa.me/${WHATSAPP_NUMBER}?text=${msg}`;
  }

  const heroWaBtn = qs('#hero-wa-btn');
  if (heroWaBtn) {
    const msg = isAr
      ? encodeURIComponent("مرحباً شركة الذهبي لمستلزمات الأسنان، أود الاطلاع على أحدث العروض والمنتجات.")
      : encodeURIComponent("Hello Al-Dahabi Dental Supplies, I would like to check current offers and products.");
    heroWaBtn.href = `https://wa.me/${WHATSAPP_NUMBER}?text=${msg}`;
  }

  const wholesaleLink = qs('#footer-link-wholesale');
  if (wholesaleLink) {
    const msg = isAr
      ? encodeURIComponent("مرحباً، أود الاستفسار عن عروض أسعار الجملة للعيادات.")
      : encodeURIComponent("Hello, I would like to request wholesale clinic pricing.");
    wholesaleLink.href = `https://wa.me/${WHATSAPP_NUMBER}?text=${msg}`;
  }

  const maintLink = qs('#footer-link-maintenance');
  if (maintLink) {
    const msg = isAr
      ? encodeURIComponent("مرحباً، أود الاستفسار عن خدمات الصيانة وتركيب الأجهزة.")
      : encodeURIComponent("Hello, I would like to inquire about dental equipment maintenance & installation.");
    maintLink.href = `https://wa.me/${WHATSAPP_NUMBER}?text=${msg}`;
  }

  const ambLink = qs('#footer-link-ambassadors');
  if (ambLink) {
    const msg = isAr
      ? encodeURIComponent("مرحباً، أود الاستفسار عن برنامج سفراء الجامعات لطلاب طب الأسنان.")
      : encodeURIComponent("Hello, I would like to inquire about the University Student Ambassador Program.");
    ambLink.href = `https://wa.me/${WHATSAPP_NUMBER}?text=${msg}`;
  }
...
function submitConsultation(e) {
  ...
  const defaultProduct = currentLang === 'ar' ? 'استشارة إكلينيكية عامة' : 'General Clinical Inquiry';
  const product   = activeProduct ? activeProduct.name : defaultProduct;
...
function animateCounters() {
  qsa('[data-count]').forEach(el => {
    const target = parseInt(el.dataset.count);
    const suffix = getStatSuffix(el, currentLang);
    let current = 0;
